...
// Where Omeka is. The module normally sits in modules/, so two levels up is
// the usual answer; OMEKA_PATH in the environment overrides it for a checkout
// kept somewhere else.
define('OMEKA_PATH', getenv('OMEKA_PATH') ?: dirname(__DIR__, 3));

if (!is_readable(OMEKA_PATH . '/vendor/autoload.php')) {
    fwrite(STDERR, sprintf(
        "No Omeka at %s. Set OMEKA_PATH=/path/to/omeka-s and try again.\n",
        OMEKA_PATH
    ));
    exit(1);
}
