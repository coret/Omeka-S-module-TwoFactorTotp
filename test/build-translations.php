...
$nl = [
    ...
    'Add a second step to your login — a code from an authenticator app on your phone, or a passkey such as a hardware key, Touch ID or Windows Hello — so a stolen password is not enough on its own.' => 'Voeg een tweede stap toe aan je login — een code uit een authenticator-app op je telefoon, of een passkey zoals een hardwaresleutel, Touch ID of Windows Hello — zodat een gestolen wachtwoord op zichzelf niet genoeg is.',
    'Added' => 'Toegevoegd',
    ...
    'TOTP secrets are encrypted at rest.' => 'TOTP-sleutels worden versleuteld opgeslagen.',
    'TOTP secrets are stored unencrypted, so read access to the database alone is enough to bypass the second factor. To encrypt them, set twofactortotp.encryption_key in Omeka\'s config/local.config.php — existing enrollments convert themselves as their owners log in.' => 'TOTP-sleutels worden onversleuteld opgeslagen, waardoor alleen leestoegang tot de database al genoeg is om de tweede factor te omzeilen. Stel twofactortotp.encryption_key in in config/local.config.php van Omeka om ze te versleutelen — bestaande registraties worden vanzelf omgezet zodra de eigenaar inlogt.',
    ...
    'This removes the authenticator app from your account and signs out every device you had marked as trusted. If it was your last second factor your recovery codes go with it, and your account will be protected by its password alone.' => 'Hiermee verwijder je de authenticator-app van je account en worden alle apparaten die je als vertrouwd had gemarkeerd uitgelogd. Als het je laatste tweede factor was, gaan je herstelcodes mee en wordt je account daarna alleen nog door het wachtwoord beschermd.',
    ...
    'Two-factor authentication, all passkeys, all recovery codes and all trusted devices will be removed for %s. They will be emailed about it, and the action is written to the log.' => 'Tweefactorauthenticatie, alle passkeys, alle herstelcodes en alle vertrouwde apparaten worden verwijderd voor %s. Diegene krijgt hierover een e-mail en de actie wordt in het log vastgelegd.',
    ...
    'Users with a selected role are held on the setup page, anywhere on the site, until they have enrolled in a second factor. Any factor satisfies it. Existing sessions are not affected.' => 'Gebruikers met een geselecteerde rol worden overal op de site op de instelpagina gehouden totdat ze een tweede factor hebben ingesteld. Elke factor voldoet. Bestaande sessies worden niet beïnvloed.',
    'Failed attempts before the account is locked' => 'Mislukte pogingen voordat het account wordt vergrendeld',
    'Counted per account rather than per login, so starting a new login does not hand out a fresh set of guesses. Recovery codes are never locked, so nobody is left with no way in. Set to 0 to turn the lockout off.' => 'Geteld per account in plaats van per login, zodat een nieuwe login geen nieuwe reeks pogingen oplevert. Herstelcodes worden nooit vergrendeld, zodat niemand zonder toegang achterblijft. Zet op 0 om de vergrendeling uit te schakelen.',
    'How long the account stays locked (seconds)' => 'Hoe lang het account vergrendeld blijft (seconden)',
    'The length of the first lockout. Each further lockout on the same account is twice as long as the one before, up to a day.' => 'De duur van de eerste vergrendeling. Elke volgende vergrendeling van hetzelfde account duurt twee keer zo lang als de vorige, tot maximaal een dag.',
    'Too many failed attempts. This account cannot use a code for another %d minutes. A recovery code still works.' => 'Te veel mislukte pogingen. Dit account kan nog %d minuten geen code gebruiken. Een herstelcode werkt wel.',
    'You are asked for your passkey when you log in.' => 'Er wordt om je passkey gevraagd wanneer je inlogt.',
