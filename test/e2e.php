...
function http(string $url, ?array $post = null, bool $follow = true, array $headers = []): array
{
    global $jar;
    $ch = curl_init($url);
...
$passkeys = http(BASE . '/admin/two-factor/passkeys');
check('passkeys page is held behind the password', str_contains($passkeys['body'], 'type="password"'), $passkeys['url']);
$adminChallenge = http(BASE . '/admin/two-factor/passkey-challenge', [], true, ['X-Requested-With: XMLHttpRequest']);
check(
    'passkey registration is refused before the password is confirmed',
    403 === $adminChallenge['code'],
    (string) $adminChallenge['code']
);
$confirmed = http(BASE . '/admin/two-factor/passkeys', hidden($passkeys['body']) + ['password' => $password]);
check('the right password opens the passkeys page', str_contains($confirmed['body'], 'totp-passkey-register'), $confirmed['url']);
...
// These endpoints only answer a same-origin XHR now, so the header that a
// cross-origin request cannot set without a preflight has to be sent.
const XHR = ['X-Requested-With: XMLHttpRequest'];

// The CSRF barrier itself: no header, no answer, whatever else is true.
foreach (['challenge', 'verify'] as $action) {
    $plain = http(BASE . '/two-factor/passkey/' . $action, [], true, []);
    check(
        "$action refuses a request that is not a same-origin XHR",
        400 === $plain['code'],
        (string) $plain['code']
    );
}
$get = http(BASE . '/two-factor/passkey/challenge');
check('challenge refuses GET', 400 === $get['code'], (string) $get['code']);

$noSession = http(BASE . '/two-factor/passkey/challenge', [], true, XHR);
check('challenge without a pending login is refused', 440 === $noSession['code'], (string) $noSession['code']);
$noSessionVerify = http(BASE . '/two-factor/passkey/verify', [], true, XHR);
...
$noCreds = http(BASE . '/two-factor/passkey/challenge', [], true, XHR);
check(
    'challenge for a user with no passkey is refused',
    409 === $noCreds['code'],
    (string) $noCreds['code']
);
$noChallenge = http(BASE . '/two-factor/passkey/verify', [], true, XHR);
check(
